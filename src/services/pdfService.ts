...
export const generateQuotationPDF = (data: QuotationData) => {
  try {
    ...
    doc.text('+966 53 763 3390', 105, 215, { align: 'center' });
    doc.text('المملكة العربية السعودية الرياض الدمام جدة مكه المدينة تبوك جيزان', 105, 230, { align: 'center' });

    const blob = doc.output('blob');
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `Quotation_${data.quotationNumber}.pdf`;
