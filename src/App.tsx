          ...
          >
            <div className="liquid-glass rounded-3xl p-8 flex flex-col gap-6 shadow-2xl">
              {navItems.map((item) => (
                ...
                >
                  <span className="group-hover:text-safety-red transition-colors">{item.name}</span>
                  <ChevronRight size={20} className={`text-white/20 group-hover:text-safety-red transition-all ${i18n.language === 'ar' ? 'rotate-180 group-hover:-translate-x-1' : 'group-hover:translate-x-1'}`} />
                </motion.a>
              ))}
...
const Hero = () => {
  ...
  return (
    <section className="relative min-h-screen flex items-center pt-40 md:pt-48 pb-20 overflow-hidden">
      {/* Background Elements */}
      <div className={`absolute top-1/4 ${i18n.language === 'ar' ? '-right-20' : '-left-20'} w-96 h-96 bg-safety-red/20 blur-[120px] rounded-full`} />
      <div className={`absolute bottom-1/4 ${i18n.language === 'ar' ? '-left-20' : '-right-20'} w-96 h-96 bg-security-blue/20 blur-[120px] rounded-full`} />
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-20 gap-8">
          <div className="max-w-2xl">
            <h2 className="text-sm font-bold tracking-[0.3em] uppercase text-safety-red mb-4">{t('services.expertise')}</h2>
